...
public class ImageInfo
{
    ...
    */
    ...
    public void storeDiskInMongo(String diskName) throws Exception {
    	MongoClient mongoClient = new MongoClient();
    	MongoDatabase db = mongoClient.getDatabase("photo");
    	MongoCollection<org.bson.Document> inventory = db.getCollection("inventory");
    	
    	JSONObject mainDoc = new JSONObject();
    	JSONArray images = new JSONArray();
    	mainDoc.put("images", images);
    	
    	for (ImageInfo ii : imagesByName) {
    		if (diskName.equals(ii.diskMgr.diskName)) {
    			images.put(ii.getJSON());
    		}
    	}

    	String jsonVersion = mainDoc.toString(2);
    	org.bson.Document doc = Document.parse(jsonVersion);
