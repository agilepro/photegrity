...
public class NNTPClient extends NNTP
{
    ...
    throws IOException
    {
        if (articleId != null)
        {
            int res = sendCommand(command, articleId);
            if (!NNTPReply.isPositiveCompletion(res)) {
                throw new IOException("isPositiveCompletion returned false for command '"+command+"' and article '"
                    +articleId+"' and result '"+res+"'");
            }
        }
        else
        {
            ...
            if (!NNTPReply.isPositiveCompletion(res)) {
                throw new IOException("isPositiveCompletion returned false for command '"+command+"' and result '"+res+"'");
            }
        }
    ...
    throws IOException
    {
        int res = sendCommand(command,Long.toString(articleNumber));
        if (res==412) {
            throw new IOException("The server responded with 412 meaning that no newsgroup has been selected.");
        }
        if (!NNTPReply.isPositiveCompletion(res)) {
            throw new IOException("isPositiveCompletion returned false for command '"+command+"' and article '"
