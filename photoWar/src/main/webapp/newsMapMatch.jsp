...
%><%request.setCharacterEncoding("UTF-8");
    ...
    <%
    if (map!=null) {
        for (String tag : map.dest.getTags()) {
            if (!allTags.contains(tag)) {
                allTags.add(tag);
            }
        }
    }
    else if (nf!=null && npatt.hasTemplate()) {
        for (PosPat pp : npatt.getPosPatList()) {
...
<table><tr><td><a href="news.jsp?<%=startPart%>">News</a></td>
           ...
           <td><a href="newsDetail2.jsp?d=<%=URLEncoder.encode(art.getDigest(),"UTF-8")%>&f=<%=URLEncoder.encode(art.getHeaderFrom(),"UTF-8")%>">Articles</a></td></tr></table>
<hr/>

<ul>
<li> Article number: <%= art.getNumber()  %> Pattern: <%  HTMLWriter.writeHtml(out, npatt.getSampleFileName() ); %></li>
...
<%
    for (PatternInfo pi : piSet) {
        ...
        Vector<ImageInfo> sortedImages = new Vector<ImageInfo>();
        sortedImages.addAll(pi.allImages);
        ImageInfo.sortImages(sortedImages, "name");
        Enumeration<ImageInfo> e2 = sortedImages.elements();
        int totalCount=-1;
